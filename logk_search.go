...
func SearchBagsLogK(gen decomp.Generator, input, comp decomp.Graph, toCoord chan lib.MessageToCoordinator, found chan lib.MessageToCTD, workerID string, countComps int, Conn []int, allowedFull decomp.Edges, width int) {
...
func sendComponents(localComps, globalComps []decomp.Graph, Conns [][]int, allowedFull, allowedRed decomp.Edges, toCoord chan lib.MessageToCoordinator, found chan lib.MessageToCTD, workerID string, countComps int, isolated []decomp.Edge, sep decomp.Edges, H, Upper decomp.Graph, OldConn []int) {
	...
	for i := range globalComps {
		...
		out := lib.CreateBlock(sep, globalComps[i].Edges, treeComp, H.Edges, conn)
